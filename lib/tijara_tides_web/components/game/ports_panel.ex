defmodule TijaraTidesWeb.GameUI.PortsPanel do
    ...
    <section id="ports-panel" class="workspace-panel" aria-label={gettext("Ports")}>
      ...
      <div class="panel-content" tabindex="0" aria-label={gettext("Port details and trading")}>
        <section class="my-6 rounded-xl border border-slate-700 p-5">
          ...
          />
          <div id="port-market" phx-hook="PortMarketTabs" data-side={@port_market_side}>
            <div
            ...
            >
              <button
                ...
                phx-value-side={side}
                data-market-side={side}
                aria-controls={"port-market-" <> side}
            ...
            <div
              :for={side <- ["buy", "sell"]}
              id={"port-market-" <> side}
              data-market-panel={side}
              hidden={@port_market_side != side}
            >
              <.market_side {assigns} port_market_side={side} />
            </div>
          </div>
  ...
  defp market_side(assigns) do
    ...
    <div :if={market_rows != []} class="overflow-x-auto">
      <table
        id={"port-market-table-" <> @port_market_side}
        class="port-market-table w-full text-left text-sm"
        aria-label={
        ...
        <thead class="text-slate-400">
          ...
          >
            ...
            <td
              ...
              id={"aboard-#{@port_market_side}-#{String.replace(good, " ", "-")}"}
                        ...
                        if(available <= 0,
                          do:
                            if(side == "buy",
                              ...
                              else: gettext("No feasible sale: check cargo, demand, and buyer funds")
                            )
                        )
                  }
