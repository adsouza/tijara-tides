defmodule TijaraTides.Domain.EmailIdentity do
  ...
  alias TijaraTides.Domain.{Accounts, Guarantees}

  def delivered(state, row),
    do: put(state, "email_requests", row["id"], %{row | "delivery" => "sent"})
  ...
  def request(state, account, purpose, address, context) do
    with {:ok, email} <- normalize(address) do
      ...
      cond do
        ...
        purpose == "invite" and Guarantees.suspended?(account) ->
        ...
        true ->
          invited =
            if purpose == "invite",
              do: Accounts.issue_invite(state, account, %{invite_hash: context.hash}),
              else: {:ok, state, %{}}
  ...
  def redeem(state, hash, session, signed_in, context) do
    ...
    cond do
      ...
      row["used_session"] == session ->
        case Accounts.authenticate(state, session, context.wall_ms) do
        ...
        if owner && (row["purpose"] == "invite" or owner["id"] != row["account_id"]) do
          ...
          result =
            if row["purpose"] == "invite",
              do:
                Accounts.redeem(state, hash, session, Map.put(context, :email_verification, true)),
              else: {:ok, state, %{"account_id" => row["account_id"]}}

          with {:ok, state, %{"account_id" => id}} <- result do
            account = get(state, "accounts", id)

            state =
              state
              |> put("accounts", id, Map.put(account, "email", row["email"]))
              |> put("sessions", session, %{
                "account_id" => id,
                "expires_at" => context.wall_ms + 30 * 86_400_000
              })
