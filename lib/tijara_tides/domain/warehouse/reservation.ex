defmodule TijaraTides.Domain.Warehouse.Reservation do
  ...
  @enforce_keys @fields
  defstruct @fields ++ [order_id: nil]

  def from_row(row) do
    if Map.keys(row) -- Enum.map([:order_id | @fields], &Atom.to_string/1) != [],
  ...
  def to_row(r), do: Map.new([:order_id | @fields], &{Atom.to_string(&1), Map.fetch!(r, &1)})
end
