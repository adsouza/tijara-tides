defmodule TijaraTides.Domain.Ship.VisitOrders do
  ...
  def advance(state, catalogue) do
    state = TijaraTides.Domain.Ship.RoutePlan.advance(state, catalogue)
    # Stable order across restarts; sell instructions always precede purchases.
    entities(state, "ship_instructions")
    |> Map.values()
    |> Enum.filter(
      &(&1["status"] in @open and
          TijaraTides.Domain.Ship.RoutePlan.executable?(state, &1["ship_id"]))
    )
  ...
  defp depart_ready_visits(state, catalogue) do
    ...
    |> Enum.reduce(state, fn {id, plan}, state ->
      ...
      if plan["auto_depart"] == true and
           TijaraTides.Domain.Ship.RoutePlan.executable?(state, plan["ship_id"]) and
           not is_nil(ship) and
      ...
      case first do
        ...
        {:ok, _, _} ->
          ...
          order =
            ...
            |> TijaraTides.Domain.Ship.VisitOrder.record_fill(quantity, reply["spent"] || 0)
