defmodule TijaraTides.Domain.AccountWorld.EmailIdentity do
  ...
  def request(state, account, purpose, address, context) do
    with {:ok, email} <- normalize(address) do
      ...
      cond do
        ...
        purpose == "invite" and AccountWorld.suspended?(account) ->
        ...
        true ->
          invited =
            if purpose == "invite",
              do: AccountWorld.issue_invite(state, account, %{invite_hash: context.hash}),
              else: {:ok, state, %{}}
  ...
  def redeem(state, hash, session, signed_in, context) do
    ...
    cond do
      ...
      row["used_session"] == session ->
        case AccountWorld.authenticate(state, session, context.wall_ms) do
            ...
            if row["purpose"] == "invite",
              do:
                AccountWorld.redeem(
                  ...
                  hash,
                  session,
                  Map.put(context, :email_verification, true)
                ),
