      ...
      repo.query!(
        ...
        LEFT JOIN LATERAL (
          SELECT coalesce(sum(balance_cents) FILTER(WHERE account_code IN ('cash_available','cash_reserved')),0) AS cash,
           ...
           coalesce(sum(balance_cents) FILTER(WHERE account_code='prepaid_rent'),0) AS prepaid,
        ...
        WHERE c.world_id=$1 AND ($2::text[] IS NULL OR c.id=ANY($2::text[])) AND (c.cash_cents<>b.cash OR c.reserved_cents<>b.reserved OR c.unpaid_cents<>b.unpaid OR c.profit_cents<>b.profit
         ...
         OR b.reserved<(SELECT coalesce(sum(o.quantity*o.price),0) FROM game_exchange_orders o WHERE o.world_id=c.world_id AND o.company_id=c.id AND o.side='buy') + (SELECT coalesce(sum(x.amount),0) FROM game_auction_bids x JOIN game_auctions a ON a.world_id=x.world_id AND a.id=x.auction_id WHERE x.world_id=c.world_id AND x.company_id=c.id AND a.status='scheduled')
         OR EXISTS(SELECT 1 FROM game_exchange_orders o JOIN game_warehouses w ON w.world_id=o.world_id AND w.id=o.warehouse_id WHERE o.world_id=c.world_id AND o.company_id=c.id AND (w.company_id<>o.company_id OR w.port<>o.port OR NOT EXISTS(SELECT 1 FROM game_warehouse_reservations r WHERE r.world_id=o.world_id AND r.order_id=o.id AND r.warehouse_id=o.warehouse_id AND r.company_id=o.company_id AND r.good=o.good AND r.quantity=o.quantity AND r.kind=CASE WHEN o.side='buy' THEN 'capacity' ELSE 'stock' END)))
         OR EXISTS(SELECT 1 FROM game_auctions a JOIN game_warehouses w ON w.world_id=a.world_id AND w.id=a.warehouse_id WHERE a.world_id=c.world_id AND a.company_id=c.id AND a.status='scheduled' AND (w.company_id<>a.company_id OR w.port<>a.port OR #{@covered_until}<a.closes_ms OR NOT EXISTS(SELECT 1 FROM game_warehouse_reservations r WHERE r.world_id=a.world_id AND r.auction_id=a.id AND r.company_id=a.company_id AND r.warehouse_id=a.warehouse_id AND r.good=a.good AND r.kind='stock' AND r.quantity=a.quantity)))
         OR EXISTS(SELECT 1 FROM game_auction_bids x JOIN game_auctions a ON a.world_id=x.world_id AND a.id=x.auction_id JOIN game_warehouses w ON w.world_id=x.world_id AND w.id=x.warehouse_id WHERE x.world_id=c.world_id AND x.company_id=c.id AND a.status='scheduled' AND (w.company_id<>x.company_id OR w.port<>a.port OR #{@covered_until}<a.closes_ms OR x.company_id=a.company_id OR NOT EXISTS(SELECT 1 FROM game_warehouse_reservations r WHERE r.world_id=x.world_id AND r.bid_id=x.id AND r.company_id=x.company_id AND r.warehouse_id=x.warehouse_id AND r.good=a.good AND r.kind='capacity' AND r.quantity=a.quantity)))
         OR EXISTS(SELECT 1 FROM game_auctions a LEFT JOIN game_ships s ON s.world_id=a.world_id AND s.id=a.ship_id WHERE a.world_id=c.world_id AND a.company_id=c.id AND a.ship_id IS NOT NULL AND a.status='scheduled' AND (s.id IS NULL OR s.company_id<>a.company_id OR s.status<>'docked' OR EXISTS(SELECT 1 FROM game_cargo_holdings h WHERE h.world_id=s.world_id AND h.ship_id=s.id)))
         OR EXISTS(SELECT 1 FROM game_auction_bids x JOIN game_auctions a ON a.world_id=x.world_id AND a.id=x.auction_id LEFT JOIN game_companies seller ON seller.world_id=a.world_id AND seller.id=a.company_id WHERE x.world_id=c.world_id AND x.company_id=c.id AND a.status='scheduled' AND ((a.ship_id IS NULL AND x.warehouse_id IS NULL) OR (a.ship_id IS NOT NULL AND x.warehouse_id IS NOT NULL) OR (seller.bankruptcy_ms IS NOT NULL AND seller.account_id=c.account_id)))
         OR b.prepaid<>(SELECT coalesce(sum(prepaid+next_rent),0) FROM game_warehouses w WHERE w.world_id=c.world_id AND w.company_id=c.id)
