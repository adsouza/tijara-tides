...
changed, confirm nothing was added, and regenerate. The keyword list covers
inflected forms deliberately: an earlier version matched `show` but not `shows`,
so rewording a requirement to be more imperative silently reduced the count.

Heuristic requirement sentences in DESIGN.md at generation: 62
...
- Sections 3 and 7 both govern how time is shown, without referencing each
  other. Section 3 requires a game period and its real interval together wherever
  a player commits; section 7 requires auction opening and closing times in the
  player's local time with countdowns. An auction window is one place a player
  commits, so both apply and neither says how they combine.
...
- Progressive disclosure was absent as a principle, so nothing prevented an
  implementation surfacing earmarked budgets or markdown schedules on the path to
  a first purchase. Section 1 now states it, bounded so that it never defers a
  required pre-commitment disclosure, and decision group 24 records it.

- Requirement 4 was permissive where its neighbours are imperative, leaving the
  floor unstated even though the privacy ceiling was clear. Section 4 now requires
  company and ship class on inspection.
