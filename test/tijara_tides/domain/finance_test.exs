defmodule TijaraTides.Domain.FinanceTest do
  ...
  test "borrowing is a liability, not profit, with conservative available credit", c do
    ...
    assert Finance.summary(state, c.account)["available"] == before["available"] - 100_000

    assert [%{entries: [{"cash_available", 100_000}, {"loan_principal", -100_000}]}] =
             state.journal

    assert length(hd(Finance.summary(state, c.account)["loans"])["schedule"]) == 4
  end
  ...
  test "invalid loans and excessive credit cannot move money", c do
    for amount <- [0, -1, nil, "100", 99],
        do:
          assert(
            {:error, :loan_invalid_amount} == Finance.borrow(c.state, c.account, amount, "x")
          )
    ...
    assert {:error, :loan_limit} =
             Finance.borrow(
               c.state,
               c.account,
               Finance.summary(c.state, c.account)["available"] + 1,
               "x"
             )

    assert {:error, :finance_no_company} =
             Finance.borrow(c.state, %{"company_id" => nil}, 100, "x")

    state =
      Enum.reduce(1..8, c.state, fn n, s ->
        {:ok, s, _} = Finance.borrow(s, c.account, 100, to_string(n))
        s
      end)

    assert {:error, :loan_count_limit} = Finance.borrow(state, c.account, 100, "ninth")
  end

  test "installments accrue and settle once with declining interest", c do
    {:ok, state, _} = loan(c)
    period = Finance.terms().period_ms
    ...
    assert Finance.settle(state) == state
  ...
  test "early repayment waives future interest and cannot farm credit increases", c do
    ...
    {:ok, state, _} = loan(c)
    assert {:error, :loan_not_owned} = Finance.repay(state, %{"company_id" => "other"}, "loan")
    {:ok, state, %{"repaid" => 100_000}} = Finance.repay(state, c.account, "loan")
    assert state.entities["companies"]["company"]["profit"] == 0
    assert Finance.summary(state, c.account)["limit"] == limit
    assert {:ok, _, %{"repaid" => 0}} = Finance.repay(state, c.account, "loan")

    assert Finance.settle(%{state | clock_ms: 10 * Finance.terms().period_ms}).entities["loans"][
             "loan"
           ]["interest_due"] == 0
  end
  ...
  test "continuous interest is independent of tick size, not overdue before its due date, and paid on early closure",
    ...
    assert hd(hd(Finance.summary(halfway, c.account)["loans"])["schedule"])["interest"] == 8000
    assert Finance.summary(halfway, c.account)["arrears"] == 0
    assert Finance.summary(halfway, c.account)["deadline"] == nil
    assert Finance.settle(halfway) == halfway
    assert {:ok, repaid, %{"repaid" => 104_000}} = Finance.repay(halfway, c.account, "loan")
  ...
  test "protected funds cannot service debt; partial payments retain the original grace deadline",
    ...
    assert {:error, :loan_repayment_funds} = Finance.repay(state, c.account, "loan")
    period = Finance.terms().period_ms
    waiting = tick(state, period)
    assert waiting.entities["companies"]["company"]["cash"] == cash
    assert Finance.summary(waiting, c.account)["available"] == 0
    assert Finance.settle(waiting) == waiting
    deadline = Finance.summary(waiting, c.account)["deadline"]
    ...
    assert Finance.summary(partial, c.account)["deadline"] == deadline

    recovered =
      put_in(partial, [:entities, "companies", "company", "reserved"], 0) |> Finance.settle()

    assert Finance.summary(recovered, c.account)["deadline"] == nil
  end
  ...
  test "operating bills and installments compete oldest due first", c do
    ...
    state =
      ...
      |> Finance.operating_bill("company", 1000, period - 1)
      |> Finance.operating_bill("company", 1000, period + 1)
  ...
  test "forced bankruptcy uses only active time and never transfers old assets", c do
    ...
    due = Finance.terms().period_ms
    waiting = tick(state, due)
    assert Finance.settle(waiting) == waiting
    state = tick(waiting, due + Finance.terms().grace_ms)
    ...
    assert {:error, :finance_no_company} = Finance.bankrupt(state, c.account)
  ...
  test "voluntary bankruptcy cancels orders but committed sailing finishes", c do
    ...
    {:ok, state, _} = Finance.bankrupt(state, c.account)
  ...
  test "voluntary bankruptcy requires liabilities to exceed unreserved cash", c do
    assert {:error, :bankruptcy_cash_covers_debts} = Finance.bankrupt(c.state, c.account)
    {:ok, state, _} = loan(c)
    state = put_in(state, [:entities, "companies", "company", "cash"], 100_000)
    refute Finance.can_declare_bankruptcy?(state, c.account)
    assert {:error, :bankruptcy_cash_covers_debts} = Finance.bankrupt(state, c.account)
    reserved = put_in(state, [:entities, "companies", "company", "reserved"], 1)
    assert Finance.can_declare_bankruptcy?(reserved, c.account)
    assert {:ok, _, _} = Finance.bankrupt(reserved, c.account)
    accrued = tick(state, div(Finance.terms().period_ms, 2))
